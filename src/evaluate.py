...
class Evaluate:
    ...
    def __init__(self, env, oracle, policies, n_rollouts=500, policy_names=None):
        ...
        self.policy_names = policy_names if policy_names else [f"policy{i}" for i in range(len(policies))]
        print(f'getting {n_rollouts} rollouts')
    ...
    def play_performance(self):
        ...
        print(F"Oracle average: {self._oracle_average_reward()} with std: {self._oracle_std()}")
        for avg, std, name, trace in zip(self._policy_average_rewards(), self._policy_stds(), self.policy_names, self.policy_traces):
            ...
            print(f"{name} avg: {avg:.2f} with std {std:.2f}. Difference = {diff:.2f} and min = {min_rollout}")
    ...
    def fidelity(self):
        ...
        for name, policy, trace in zip(self.policy_names, self.policies, self.flattened_policy_traces):
            ...
            print(f'{name} fidelity in oracle trace {(fid1 * 100):.1f}%')
            print(f'{name} fidelity in policy trace {(fid2 * 100):.1f}%')
