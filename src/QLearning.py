    ...
    def train(self, env):
        ...
        for episode_index in range(self.env_config.n_episodes):
            ...
            if episode_index % 100 == 0:
                print(f"Episode {episode_index} average reward: {sum(rewards) / len(rewards)}; epsilon = {self.epsilon:.4f}, lr = {self.lr:.6f}")
                rewards = []
        env.close()

    @staticmethod
    def get_oracle(env, specifier=""):
        path = Path('QLearning', env.unwrapped.spec.id, f'oracle{specifier}.pk')
        if path.exists():
